...
const StoryPlanner = () => {
  ...
  const handleBulkGenerate = () => {
    ...
    // UPDATED REGEX: Split by "Story 1" or "Story 1 :" etc. 
    // Removed implicit double newline split to respect user wish.
    const slideTexts = bulkText.split(/(?:Story|Slide|Sequenz)\s*\d+\s*[:.-]?/i).filter(t => t.trim().length > 0);
    
    if (slideTexts.length === 0) {
        // Fallback: If no markers found, treat as one big story or try lines? 
        // User requested "Empty lines are NO separator". So we assume one slide if no markers.
        // But for convenience, if there are NO markers at all, maybe we should just use the whole text.
    ...
    const newSlides = slideTexts.map((text, index) => {
      ...
      const layouts = ['story_text_box', 'editorial_mask', 'diagonal_overlay', 'minimal_quote'];
